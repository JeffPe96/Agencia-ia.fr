              ...
              {plans.map((plan, i) => (
                <ScrollReveal key={plan.name} delay={i * 120}>
                  <div className={`card-glass h-full flex flex-col relative ${plan.popular ? "border-primary/30 shadow-[0_0_30px_-8px_hsl(217_91%_53%/0.15)]" : ""}`}>
                    ...
                    <button
                      onClick={() => handleCta(plan.target)}
                      className={`w-full text-center text-sm font-medium px-6 py-3 rounded-xl flex items-center justify-center gap-2 transition-all cursor-pointer ${
                        plan.popular
                          ? "btn-primary-neu"
